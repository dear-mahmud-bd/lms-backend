...
export interface ApiBlogPostBlogPost extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'A blog article. Publish state is the explicit `status` enum (not Strapi draft/publish); only published posts are public.';
...
export interface ApiCourseCourse extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'A course owned by an instructor; contains lessons and quizzes.';
    displayName: 'Course';
    pluralName: 'courses';
    singularName: 'course';
  };
...
export interface ApiEnrollmentEnrollment extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'Join row: a student enrolled in a course. Carries enrolledAt and status.';
...
export interface ApiLessonLesson extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'A unit of content within a course; text and/or a video URL, ordered by `order`.';
    displayName: 'Lesson';
    pluralName: 'lessons';
    singularName: 'lesson';
  };
...
export interface ApiNotificationNotification
  ...
  info: {
    description: 'Placeholder entity (no driving requirement). Minimal generic notification for a recipient; users read/mark only their own (Phase 10).';
  ...
  attributes: {
    ...
    recipient: Schema.Attribute.Relation<
      ...
      'plugin::users-permissions.user'
    >;
    type: Schema.Attribute.Enumeration<['enrollment', 'quiz-graded', 'system']>;
    updatedAt: Schema.Attribute.DateTime;
    updatedBy: Schema.Attribute.Relation<'oneToOne', 'admin::user'> &
      Schema.Attribute.Private;
  };
...
export interface ApiProgressProgress extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'Completed-lessons join: one row = a student completed a lesson. Per-course % is computed, never stored.';
...
export interface ApiQuestionQuestion extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'An MCQ question in a quiz. correctAnswer is a 0-based index into options; never exposed to students in the taking route.';
...
export interface ApiQuizAttemptQuizAttempt extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: "One student's graded submission of one quiz. score is computed server-side at submit; multiple attempts allowed by default.";
...
export interface ApiQuizQuiz extends Struct.CollectionTypeSchema {
  ...
  info: {
    description: 'A course-level quiz. Ownership resolves via quiz.course.instructor.';
    displayName: 'Quiz';
    pluralName: 'quizzes';
    singularName: 'quiz';
  };
